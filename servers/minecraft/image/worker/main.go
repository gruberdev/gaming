...
// main intercepts the stdout of the Minecraft gameserver and uses it
...
func main() {
	...
	serverCapacity, err := strconv.ParseInt(os.Getenv("MAX_PLAYERS"), 10, 64)
	...
	if *enablePlayerTracking {
		...
		fmt.Println(">>> Starting wrapper for Minecraft!")
		fmt.Printf(">>> Path to Minecraft server script: %s %v\n", *input, argsList)
		...
		cmd.Stdout = &interceptor{
			...
			intercept: func(p []byte) {
				...
				if strings.Contains(str, "This server is running Multipaper version") {
					err = s.Ready()
					if err != nil {
		...
		err = cmd.Wait()
		log.Fatal(">>> Minecraft shutdown unexpectantly", err)
	}
}
...
func handleLogLine(line string) (string, *string) {
	...
	playerJoin := regexp.MustCompile(`^\[\d{2}:\d{2}:\d{2}\sINFO]: (.+) joined the game`)
	playerLeave := regexp.MustCompile(`^\[\d{2}:\d{2}:\d{2}\s Multipaper versionINFO]: (.+) left the game`)
	serverStart := regexp.MustCompile(`^\[\d{2}:\d{2}:\d{2} INFO\]: Done \(\d+\.?\d*s\)! For help, type "help"`)

	// Start the server
	if serverStart.MatchString(line) {
		fmt.Println(">>> Server is ready to accept player connections!")
		return "READY", nil
	}
